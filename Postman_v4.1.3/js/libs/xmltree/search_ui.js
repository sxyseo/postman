...
(function(){
	...
	function searchXMLNodeValue(query) {
		$("span.xv-text,span.xv-tag-name").each(function() {
			if(this.innerHTML.toLowerCase().indexOf(query.toLowerCase())!=-1) {
				var ih = this.innerHTML;
                //one-in-a-million case:
                while(ih[0]==='<' && ih[ih.length-1]=='>') {
                    ih=$(ih).html();
                }
	...
	function handleKeyEvent(evt) {
        ...
        if(evt.keyCode==13) {
            evt.preventDefault();
            $(".xv-search-result-item").remove();
            performSearch();
            $(".xv-search-field").val("");

            if($(".xv-search-result-item").length==0) {
                $(".xv-search-panel").hide();
                $("#prettySearchToggle").removeClass("active");
            }
