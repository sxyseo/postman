	...
	return {
		...
		toXml: function(text) {
			...
			if (!result || !result.documentElement
				...
				throw "<h2>Cannot parse XML document</h2> \n" + (error ? error.textContent : '');
			}
		...
		formatURL: function(url) {
            //if url begins with /, it is a relative URL
            if(url[0]=='/') {
                return '<a href="' + url + '" class="xv-url cm-link">' + this.escapeHTML(url) + '</a>';
            }
			...
			return '<a href="' + href + '" class="xv-url cm-link">' + this.escapeHTML(url) + '</a>';
		}
	};
})();